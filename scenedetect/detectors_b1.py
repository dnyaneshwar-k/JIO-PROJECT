...
# specs = [
#     ('threshold', float32),               # a simple scalar field
#     ('min_scene_len', float32)          # an array field
# ]
...
class ContentDetector(SceneDetector):
    ...
    def process_frame(self, frame_num, frame_img, frame_metrics, scene_list):
        ...
        if self.last_frame is not None:
            cut_detected = content_process_frame(frame_num, frame_img, frame_metrics, scene_list,self.threshold,self.min_scene_len,self.last_frame,self.last_scene_cut,self.last_hsv)
...
def content_process_frame(frame_num, frame_img, frame_metrics, scene_list,threshold,min_scene_len,last_frame,last_scene_cut,last_hsv):
    ...
    if delta_hsv_avg >= threshold:
        if last_scene_cut==0 or (
          (frame_num - last_scene_cut) >= min_scene_len):
            scene_list.append(frame_num)
            last_scene_cut = frame_num
            cut_detected = True
